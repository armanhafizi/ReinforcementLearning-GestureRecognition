...
def create_dataloader_env(batch_size=256):
    labels = torch.from_numpy(np.load('label.npy')).long()
    windows = torch.from_numpy(np.load('chunks.npy')).float()
    next_window = torch.from_numpy(np.load('next_chunks.npy')).float()
    seq_num = torch.from_numpy(np.load('sequence.npy')).float()
    rewards = torch.ones_like(labels).float()
    ...
    tensor_dataset = TensorDataset(windows,
                                    labels,
                                    rewards[:, None],
                                    next_window,)
                                    # tensors['dones'][:, None],)
...
def train(config, seed, algo='cqldqn'):
    ...
    last10_stats = []
    dataloader = create_dataloader_env(batch_size=config.batch_size)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    avg_last10 = deque(maxlen=10)

    if algo == 'cqldqn':
        agent = CQLDQN(state_size=config.state_size,
                    action_size=config.n_classes,
        ...
        agent = DeepQN(state_size=config.state_size,
                    action_size=config.n_classes,
                    hidden_size=config.hidden_size,
                    device=device,
                    lr=config.lr,
